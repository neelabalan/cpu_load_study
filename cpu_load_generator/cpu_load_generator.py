...
import psutil
from interface import from_profile
from interface import load_all_cores
from interface import load_single_core


def parse_args(parser: argparse.ArgumentParser) -> argparse.Namespace:
    parser.add_argument(
        "-p",
        "--path_to_profile_json",
        type=str,
        default="",
...
def input_error_handler(args: argparse.Namespace):
    """Handle input errors"""
    ...
    if args.path_to_profile_json == "":
        if not args.cpu_core < cpu_count:
            args.print_help()
            raise ValueError(
                "Core to load should not be higher than {}!".format(cpu_count - 1)
            )
        if args.duration <= 0:
            args.print_help()
        ...
        if ("-c" in input_arguments or "--cpu_core" in input_arguments) and (
            ...
            args.print_help()
            raise ValueError(
                "Using any of arguments with conjunction with path_to_profile_json is disallowed!"
            )
